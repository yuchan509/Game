...
while not isSuccess: 
  ...
  # 정답을 맞추고자 값을 입력하였다->시도횟수(tryCnt) 증가
  tryCnt += 1
  # TODO step6
  '''
  ...
  if isSuccess: # 정답을 맞췄다면
    ...
    # 참이면 0, 거짓이면 (10 - tryCnt)*10    
    point = 0 if tryCnt > 10 else (10 - tryCnt)*10

    #"총 5회 시도 해서 50점을 획득하였습니다. 다시하시겠습니까?"
    print(f'총 {tryCnt}회 시도 해서 {point}점을 획득하였습니다. 다시하시겠습니까?')
    #yes:다시 게임시작-> step5, no:게임 종료
    #무조건 yes or no or 잘못된입력 3케이스만 잡겟다
    while True:
      # lower() : 소문자 처리, upper() : 대문자 처리
      choice = input( 'yes:다시 게임시작, no:게임 종료' ).strip().lower()
      # 사용자가 대문자를 입력하던, 소문자 입력, 혼용하던 다 ok
      # 사용자의 입력 형태에 상관없이 무조건 소문자로 처리하겠다
